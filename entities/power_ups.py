...
class PowerUpSystem:
    ...
    def initialize(self, board_state):
        ...
        if empty_cells:
            cells_for_power_ups = random.sample(
                empty_cells, min(num_power_ups, len(empty_cells))
            )
    ...
    def render(self, screen, board_offset, cell_size):
        ...
        for (row, col), power_up_type in self.power_ups.items():
            ...
            # Get power-up color
            color = next(
                (clr for typ, clr in self.power_up_types if typ == power_up_type),
                (255, 255, 255),
            )
            ...
            if power_up_type == "extra_move":
                ...
                pygame.draw.line(screen, color, (x, y), (x, y - hand_length), 2)
                ...
                pygame.draw.line(
                    screen, color, (x - line_length, y), (x + line_length, y), 3
                )
                pygame.draw.line(
                    screen, color, (x, y - line_length), (x, y + line_length), 3
                )
                ...
                pygame.draw.circle(
                    screen, WHITE, (x - offset, y - offset), size_with_pulse * 0.6
                )
                pygame.draw.circle(
                    screen, WHITE, (x + offset, y + offset), size_with_pulse * 0.6
                )
