...
class StrategyLevel(BaseLevel):
    ...
    def set_difficulty(self, level):
        ...
        self.cell_size = min(60, (min(SCREEN_WIDTH, SCREEN_HEIGHT) - 150) // self.board_size)
        self.board_offset = (
            (SCREEN_WIDTH - self.board_size * self.cell_size) // 2,
            (SCREEN_HEIGHT - self.board_size * self.cell_size) // 2 + 20
        )
        ...
        if hasattr(self, 'power_ups'):
            self.power_ups = PowerUpSystem(self.board_size, self.board_state, self.power_up_frequency)
    ...
    def handle_event(self, event):
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            ...
            if self.current_turn == self.player_piece and not self.game_over and not self.ai_thinking:
    ...
    def make_move(self, from_cell, to_cell):
        ...
        if self.show_visualization:
            self.pruning_data = self.ai.get_pruning_stats(self.board_state, self.ai_piece, self.player_piece)
    ...
    def apply_power_up(self, power_up_type, row, col):
        ...
        if power_up_type == "extra_move":
            ...
            self.board_state[row][col] = self.player_piece + 10  # Special state for shielded
            ...
            for adj_row, adj_col in adjacent_cells:
                if (0 <= adj_row < self.board_size and 
                    ...
                    self.board_state[adj_row][adj_col] is None):
    ...
    def get_valid_moves(self, row, col):
        ...
        for d_row, d_col in directions:
            ...
            if (0 <= new_row < self.board_size and 
                ...
                elif ((piece == self.player_piece and self.board_state[new_row][new_col] == self.ai_piece) or
                      (piece == self.ai_piece and self.board_state[new_row][new_col] == self.player_piece)):
    ...
    def get_cell_at_pos(self, pos):
        ...
        if (board_x <= x < board_x + self.board_size * self.cell_size and
            board_y <= y < board_y + self.board_size * self.cell_size):
    ...
    def update(self, dt):
        ...
        # Handle AI turn
        if self.current_turn == self.ai_piece and self.ai_thinking and not self.game_over:
    ...
    def render(self, screen):
        ...
        # Draw title
        title = self.title_font.render(f"Strategy Game - Level {self.difficulty}", True, WHITE)
        ...
        if self.show_visualization and self.pruning_data and not self.game_over:
            vis_rect = pygame.Rect(SCREEN_WIDTH // 2 - 350, SCREEN_HEIGHT - 220, 700, 180)
        ...
        if self.ai_thinking:
            ...
            screen.blit(thinking_text, (SCREEN_WIDTH // 2 - thinking_text.get_width() // 2, 150))
    ...
    def _draw_board(self, screen):
        ...
        board_rect = pygame.Rect(self.board_offset[0], self.board_offset[1], board_width, board_height)
            ...
            for col in range(self.board_size):
                ...
                if piece is not None:
                    ...
                    if piece == self.player_piece:
                        ...
                        pygame.draw.circle(screen, GREEN, (center_x, center_y), radius)
                        pygame.draw.circle(screen, (200, 200, 0), (center_x, center_y), radius, 3)
    ...
    def _draw_game_over_message(self, screen):
        ...
        # Draw instruction
        instruction = self.large_font.render("Press ESC to return to level select or Reset to play again", True, WHITE)
