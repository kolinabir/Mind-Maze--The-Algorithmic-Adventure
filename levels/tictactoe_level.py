...
class TicTacToeLevel(BaseLevel):
    def __init__(self, game_engine, asset_manager):
        ...
        self.board = GameBoard(
            self.board_size, (SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2), self.special_tiles
        )
    ...
    def make_move(self, row, col, symbol):
        ...
        if self.board.is_empty(row, col) and not self.game_over:
            ...
            extra_move = False
            if special_type:
                extra_move = self.apply_special_tile_effect(
                    special_type, row, col, symbol
                )
            ...
            if not extra_move:
                ...
                if self.current_turn == self.ai_symbol:
                    ...
                    self.visualization_data = self.ai.get_decision_tree(
                        board_state, self.ai_symbol, self.player_symbol
                    )
    ...
    def check_winner(self):
        ...
        for i in range(self.board_size):
            if all(
                board_state[i][j] == self.player_symbol for j in range(self.board_size)
            ):
        ...
        for j in range(self.board_size):
            if all(
                board_state[i][j] == self.player_symbol for i in range(self.board_size)
            ):
                self.winner = self.player_symbol
                return True
        ...
        if all(
            board_state[i][self.board_size - i - 1] == self.player_symbol
            for i in range(self.board_size)
        ):
            self.winner = self.player_symbol
            return True
        if all(
            board_state[i][self.board_size - i - 1] == self.ai_symbol
            for i in range(self.board_size)
        ):
            self.winner = self.ai_symbol
            return True
    ...
    def render(self, screen):
        ...
        title = self.title_font.render(
            f"Tic-Tac-Toe - Level {self.difficulty}", True, WHITE
        )
        ...
        if self.show_visualization and self.visualization_data:
            vis_rect = pygame.Rect(
                SCREEN_WIDTH // 2 - 250, SCREEN_HEIGHT - 230, 500, 180
            )
