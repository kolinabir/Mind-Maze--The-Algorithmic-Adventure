...
class JugLevel(BaseLevel):
    ...
    def update_jug_buttons(self):
        ...
        for i in range(len(self.capacities)):
            button_rect = pygame.Rect(start_x + i * (jug_width + 20) - jug_width // 2, 
                                      SCREEN_HEIGHT // 2 - 100, 
                                      jug_width, 180)
    ...
    def update_action_buttons(self):
        ...
        # Fill button
        fill_rect = pygame.Rect((SCREEN_WIDTH - button_width * 3 - spacing * 2) // 2, 
                               start_y, 
                               button_width, button_height)
        self.action_buttons.append(("Fill", fill_rect))
        
        # Empty button
        empty_rect = pygame.Rect(fill_rect.right + spacing, 
                                start_y, 
                                button_width, button_height)
        self.action_buttons.append(("Empty", empty_rect))
        
        # Pour button
        pour_rect = pygame.Rect(empty_rect.right + spacing, 
                               start_y, 
                               button_width, button_height)
    ...
    def perform_action(self, action):
        ...
        if action == "Fill":
            if self.jugs[self.selected_jug] < self.capacities[self.selected_jug]:
                ...
                self.pour_amount = self.capacities[self.selected_jug] - self.jugs[self.selected_jug]
                self.pour_progress = 0
    ...
    def render(self, screen):
        ...
        # Draw title and goal
        title = self.title_font.render(f"Water Jug Puzzle - Level {self.difficulty}", True, WHITE)
        screen.blit(title, (SCREEN_WIDTH // 2 - title.get_width() // 2, 30))
        
        goal = self.font.render(f"Goal: Measure exactly {self.target_amount} liter(s) in any jug", True, WHITE)
    ...
    def _draw_jugs(self, screen):
        ...
        for i, (capacity, amount) in enumerate(zip(self.capacities, self.jugs)):
            ...
            # Draw jug outline
            jug_rect = pygame.Rect(jug_x - jug_width // 2, jug_y - jug_height, jug_width, jug_height)
            ...
            if is_selected:
                ...
                pygame.draw.rect(screen, (255, 255, 0), indicator_rect, 0, border_radius=5)
                
                # Add "Selected" text above jug
                selected_text = self.font.render("Selected", True, (255, 255, 0))
                screen.blit(selected_text, (jug_x - selected_text.get_width() // 2, 
                                           jug_y - jug_height - 30))
            ...
            self.visualizer.draw_jug(
                ...
                self.pour_amount if self.is_pouring and (i == self.pour_source or i == self.pour_target) else 0,
            ...
            screen.blit(capacity_text, (jug_x - capacity_text.get_width() // 2, jug_y - jug_height - 25))
            ...
            if not is_selected and not self.is_pouring and not self.win and not self.lose:
    ...
    def _draw_completion_message(self, screen, success):
        ...
        if not success:
            retry_btn = pygame.Rect(SCREEN_WIDTH // 2 - 100, SCREEN_HEIGHT // 2 + 60, 200, 40)
