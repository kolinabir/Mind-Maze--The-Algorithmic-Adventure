...
class DecisionTreeVisualizer:
    ...
    def _calculate_layout(self, tree, rect):
        ...
        self._assign_positions(
            tree, rect.left + 10, rect.top + y_offset, 0, 0, leaf_count
        )
    ...
    def _draw_nodes(self, screen, tree, rect, depth=0):
        ...
        if depth == 0 and tree["children"]:
            best_score = (
                max(child["score"] for child in tree["children"])
                if tree["is_maximizing"]
                else min(child["score"] for child in tree["children"])
            )
            best_children = [
                child for child in tree["children"] if child["score"] == best_score
            ]
        ...
        if tree["move"] is not None:
            move_text = self.font.render(
                f"{tree['move'][0]},{tree['move'][1]}", True, (255, 255, 255)
            )
            move_rect = move_text.get_rect(
                center=(tree["x"], tree["y"] - node_radius - 10)
            )
        ...
        if hasattr(tree, "is_best") and tree["is_best"]:
            # Animated highlight
            highlight_radius = (
                node_radius + 3 + 2 * math.sin(self.animation_timer * 2 * 3.14159)
            )
            pygame.draw.circle(
                screen, (255, 255, 0), (tree["x"], tree["y"]), highlight_radius, 2
            )
