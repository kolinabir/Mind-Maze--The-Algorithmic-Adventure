...
class CompositeVisualizer:
    ...
    def _draw_minimax_tree(self, screen, rect, tree_data):
        ...
        if children:
            ...
            for i, child in enumerate(children[: int(rect.width // node_width)]):
                ...
                pygame.draw.circle(
                    screen, node_color, (node_x + node_width // 2, node_y), 10
                )
                ...
                screen.blit(
                    score_text,
                    (
                        node_x + node_width // 2 - score_text.get_width() // 2,
                        node_y + 15,
                    ),
                )
    ...
    def _draw_alpha_beta_tree(self, screen, rect, tree_data):
        ...
        if events:
            ...
            pygame.draw.line(
                screen, RED, (alpha_x, range_rect.top), (alpha_x, range_rect.bottom), 2
            )
            ...
            pygame.draw.line(
                screen, BLUE, (beta_x, range_rect.top), (beta_x, range_rect.bottom), 2
            )
    ...
    def render_pruning(self, screen, rect, pruning_data):
        ...
        efficiency = pruning_data.get("pruned_nodes", 0) / max(
            pruning_data.get("total_nodes", 1), 1
        )
        ...
        pygame.draw.rect(
            ...
            (
                meter_rect.left,
                meter_rect.top,
                int(meter_rect.width * efficiency),
                meter_rect.height,
            ),
        )
