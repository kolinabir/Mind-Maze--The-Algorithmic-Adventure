...
class WaterVisualizer:
    ...
    def draw_jug(self, screen, rect, capacity, amount, pour_progress, pour_amount, 
                 is_source, is_target, border_color, border_width):
        """
        ...
        if water_height > 0:
            ...
            if random.random() < 0.02 and water_height > 10:  # Only add bubbles if there's enough water
    ...
    def _draw_pouring(self, screen, rect, water_level, is_out, progress):
        ...
        if is_out:
            ...
            rect_size = 80
            pygame.draw.arc(screen, self.water_color, 
                          (start_x - rect_size // 2, start_y - rect_size // 2, 
            ...
            for i in range(10):
                ...
                pygame.draw.circle(screen, self.water_color, (int(x), int(y)), int(size))
        else:
    ...
    def _update_bubbles(self, dt):
        ...
        for bubble in self.bubbles[:]:
            ...
            if bubble['y'] <= bubble['water_level'] or not bubble['jug_rect'].collidepoint(bubble['x'], bubble['y']):
