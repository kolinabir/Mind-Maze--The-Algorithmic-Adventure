...
class PruningVisualizer:
    ...
    def _draw_pruning_tree(self, screen, rect, events):
        ...
        for i, event in enumerate(events):
            ...
            norm_alpha = (min(max(alpha, min_val), max_val) - min_val) / (max_val - min_val)
            norm_beta = (min(max(beta, min_val), max_val) - min_val) / (max_val - min_val)
            ...
            if norm_alpha < norm_beta:  # Valid range
                ...
                pygame.draw.rect(screen, prune_color, 
                             (range_right, y_pos, range_left - range_right, range_height))
            
            # Draw alpha marker
            pygame.draw.line(screen, RED, 
                         (range_left, y_pos), 
                         (range_left, y_pos + range_height), 2)
            
            # Draw beta marker
            pygame.draw.line(screen, BLUE, 
                         (range_right, y_pos), 
                         (range_right, y_pos + range_height), 2)
            
            # Draw value marker
            value_x = rect.left + int(norm_value * bar_width)
            pygame.draw.line(screen, WHITE, 
                         (value_x, y_pos - 3), 
                         (value_x, y_pos + range_height + 3), 2)
            ...
            if i > 0:
                ...
                pygame.draw.line(screen, GRAY, 
                             (mid_x, curr_y), 
                             (rect.left + bar_width + 5, curr_y), 1)
