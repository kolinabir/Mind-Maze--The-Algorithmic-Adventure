...
from settings import (
    MENU_BG_COLOR, MENU_TEXT_COLOR, MENU_HIGHLIGHT_COLOR, 
    BUTTON_SIZE, PLAY_STATE, SETTINGS_STATE, CREDITS_STATE
)
...
class MainMenu:
    def __init__(self, game_engine, asset_manager):
        ...
        self.buttons = [
            ...
            Button("Settings", (center_x, start_y + button_spacing), action=self.open_settings),
            Button("Credits", (center_x, start_y + 2 * button_spacing), action=self.open_credits),
            Button("Exit", (center_x, start_y + 3 * button_spacing), action=self.exit_game)
        ]
    ...
    def render(self, screen):
        ...
        # Draw subtitle
        subtitle_surface = self.subtitle_font.render(self.subtitle_text, True, MENU_TEXT_COLOR)
        subtitle_rect = subtitle_surface.get_rect(centerx=screen.get_width() // 2, top=title_rect.bottom + 10)
