...
class SoundManager:
    ...
    def load_sound(self, name, filepath, category="effects"):
        ...
        try:
            ...
            self.sounds[name] = {"sound": sound, "category": category}
            return True
        except pygame.error:
    ...
    def set_master_volume(self, volume):
        ...
        for sound_data in self.sounds.values():
            category = sound_data["category"]
            sound_data["sound"].set_volume(
                self.master_volume * self.categories.get(category, 1.0)
            )
        ...
        if category in self.categories:
            ...
            for sound_data in self.sounds.values():
                if sound_data["category"] == category:
                    sound_data["sound"].set_volume(
                        self.master_volume * self.categories[category]
                    )
