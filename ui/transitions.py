...
class WipeTransition(Transition):
    ...
    def render(self, screen, from_surface, to_surface):
        ...
        if self.direction == "left":
            ...
            screen.blit(from_surface, (0, 0))
            screen.blit(to_surface, (0, 0), (0, 0, width, SCREEN_HEIGHT))
        elif self.direction == "right":
            # Wipe from right to left
            width = int(SCREEN_WIDTH * progress)
            offset = SCREEN_WIDTH - width
            screen.blit(from_surface, (0, 0))
            screen.blit(to_surface, (offset, 0), (offset, 0, width, SCREEN_HEIGHT))
        elif self.direction == "up":
            # Wipe from top to bottom
            height = int(SCREEN_HEIGHT * progress)
            screen.blit(from_surface, (0, 0))
            screen.blit(to_surface, (0, 0), (0, 0, SCREEN_WIDTH, height))
        elif self.direction == "down":
            # Wipe from bottom to top
            height = int(SCREEN_HEIGHT * progress)
            offset = SCREEN_HEIGHT - height
            screen.blit(from_surface, (0, 0))
            screen.blit(to_surface, (0, offset), (0, offset, SCREEN_WIDTH, height))
...
class CircleTransition(Transition):
    ...
    def render(self, screen, from_surface, to_surface):
        ...
        if self.reverse:
            ...
            pygame.draw.circle(
                mask, (0, 0, 0, 255), (SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2), radius
            )
...
class TransitionManager:
    ...
    def start_transition(
        self, from_surface, to_surface, transition_type="fade", duration=1.0
    ):
