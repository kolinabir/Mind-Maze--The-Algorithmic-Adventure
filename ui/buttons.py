...
class TextButton(Button):
    def __init__(
        ...
        hover_color=MENU_HIGHLIGHT_COLOR,
    ):
    ...
    def update(self, dt):
        ...
        for i in range(3):
            if self.current_bg_color[i] < target_color[i]:
                self.current_bg_color[i] = min(
                    target_color[i],
                    self.current_bg_color[i] + self.animation_speed * dt * 255,
                )
            elif self.current_bg_color[i] > target_color[i]:
                self.current_bg_color[i] = max(
                    target_color[i],
                    self.current_bg_color[i] - self.animation_speed * dt * 255,
                )
    ...
    def render(self, screen):
        ...
        pygame.draw.rect(
            screen, tuple(self.current_bg_color), self.rect
        )  # Convert back to tuple for drawing
...
class ImageButton(Button):
    ...
    def render(self, screen):
        ...
        scaled_image = pygame.transform.scale(
            self.current_image, (int(self.current_size[0]), int(self.current_size[1]))
        )
