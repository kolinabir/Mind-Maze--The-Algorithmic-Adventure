...
class MinimaxAI:
        ...
        for move in moves:
            ...
            for child_move in child_moves[:3]:  # Limit to 3 responses for clarity
                ...
                grandchild_node = {
                    'board': child_board,
                    'move': child_move,
                    'score': self._evaluate_board(child_board, ai_symbol, player_symbol),
                    'children': [],
                    'is_maximizing': True
                }
            ...
            if child_node['children']:
                child_node['score'] = min(child['score'] for child in child_node['children'])
            else:
                child_node['score'] = self._evaluate_board(new_board, ai_symbol, player_symbol)
                
            self.decision_tree['children'].append(child_node)
        
        # Set the root score to the maximum of children
        if self.decision_tree['children']:
            self.decision_tree['score'] = max(child['score'] for child in self.decision_tree['children'])
        
        return self.decision_tree
    
    def _minimax(self, board, depth, is_maximizing, alpha, beta, ai_symbol, player_symbol):
        """
        ...
        if is_maximizing:
            ...
            for move in available_moves:
                ...
                # Recursive minimax call
                score = self._minimax(
                    new_board, depth + 1, True, alpha, beta,
                    ai_symbol, player_symbol
                )
    ...
    def _check_winner(self, board):
        ...
        # Check rows
        for row in range(size):
            if board[row][0] is not None and all(board[row][j] == board[row][0] for j in range(size)):
                return board[row][0]
        
        # Check columns
        for col in range(size):
            if board[0][col] is not None and all(board[i][col] == board[0][col] for i in range(size)):
                return board[0][col]
    ...
    def _evaluate_board(self, board, ai_symbol, player_symbol):
        ...
        # Evaluate rows
        for row in range(size):
            score += self._evaluate_line([board[row][col] for col in range(size)], ai_symbol, player_symbol)
        
        # Evaluate columns
        for col in range(size):
            score += self._evaluate_line([board[row][col] for row in range(size)], ai_symbol, player_symbol)
        
        # Evaluate main diagonal
        score += self._evaluate_line([board[i][i] for i in range(size)], ai_symbol, player_symbol)
        
        # Evaluate other diagonal
        score += self._evaluate_line([board[i][size-1-i] for i in range(size)], ai_symbol, player_symbol)
